...
export class StylePaginatorDirective implements AfterViewInit{
  ...
  constructor(
    ...
    this.rmaService.paginatorEvent.subscribe((e: PageObject) => {
      if (
      ...
      this._curPageObj = e;
      this.matPag.pageIndex = e.pageIndex;
      this.matPag.pageSize = e.pageSize;
      this.matPag.length = e.length;
      this.initPageRange();
    });
  }
      ...
      setTimeout(() => {
        for (let i = 0; i < nodeArray.length; i++) {
          if (nodeArray[i].nodeName === 'BUTTON') {
            if (nodeArray[i].disabled && !(nodeArray[i].innerHTML.length > 100 && nodeArray[i].disabled)) {
